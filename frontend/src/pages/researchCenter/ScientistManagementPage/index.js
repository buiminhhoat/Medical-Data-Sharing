...
const ScientistManagementPageStyle = styled.div`
  width: 100%;
  height: 100%;
`;

const ScientistManagementPage = () => {
  const [cookies] = useCookies(["access_token", "id", "role"]);
  const access_token = cookies.access_token;
  const id = cookies.id;
  const role = cookies.role;

  const apiGetAllScientistByResearchCenter =
    API.RESEARCH_CENTER.GET_ALL_SCIENTIST_BY_RESEARCH_CENTER;
  ...
  const fetchGetAllDoctorByMedicalInstitution = async () => {
    if (access_token) {
      ...
      try {
        const response = await fetch(apiGetAllScientistByResearchCenter, {
          method: "POST",
          headers: {
...
export default memo(ScientistManagementPage);
