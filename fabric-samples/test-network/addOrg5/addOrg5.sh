...
function generateOrg5() {
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    docker-compose -f $COMPOSE_FILE_CA_Org5 up -d 2>&1
...
function addOrg5 () {
  ...
  docker exec cli ./scripts/org5-scripts/updateChannelConfig.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
    fatalln "ERROR !!!! Unable to create config tx"
  fi

  infoln "Joining Org5 peers to network"
  docker exec cli ./scripts/org5-scripts/joinChannel.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
